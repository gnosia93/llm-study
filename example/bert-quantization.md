...
맥북에서 동작하도록 Quantization Backend 을 QNN 을 설정함.
```
...
torch.backends.quantized.engine = 'qnnpack'    ## backend 설정
...
# Evaluate interence time & accuracy
def time_model_evaluation(model, configs, tokenizer):
    ...
    print(result)
    print("Evaluate toal time (seconds): {0:.1f}".format(eval_duration_time))
